...
// Initialize Shooter
void Auton03::Initialize( RobotIO *p_pRobotIO )
{
...
void Auton03::Execute()
{
    ...
    if ( m_pRobotIO != nullptr )
    {
        ...
        {
            // Move path Logic
            m_Drivetrain->LoadPath( auton03::path1Name );
            m_Drivetrain->FollowPath();
            m_pTimeoutTimer->Reset();
            m_pTimeoutTimer->Start();

            m_eState = auton03::eState::STATE_MOVE_1;
        }

        // ****************
        // * Move 1 State *
        // ****************
        else if ( m_eState == auton03::eState::STATE_MOVE_1 )
        {
            // Move path Logic
            m_Drivetrain->FollowPath();

            // Check if path done
            if(m_Drivetrain->IsPathFinished() || (double)m_pTimeoutTimer->Get() >= auton03::dMove1Timer)
            {
                m_Drivetrain->Stop();
                m_eState = auton03::eState::STATE_DONE;
            }
        }

        else if ( m_eState == auton03::eState::STATE_DONE )
        {
            m_Drivetrain->Stop();
        }
        ...
        {
            printf("Auton_03.cpp: Error state or unknown state\n");
        }
    }
    // m_pRobotIO is nullptr
    else
    {
       // printf("Auton_03.cpp: m_pRobotIO is nullptr\n");
    }
}
